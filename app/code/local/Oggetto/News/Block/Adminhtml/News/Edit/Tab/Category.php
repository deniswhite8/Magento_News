...
/**
 * news - category relation edit block
 *
 * @category    Oggetto
 * @package     Oggetto_News
 */
class Oggetto_News_Block_Adminhtml_News_Edit_Tab_Category extends Oggetto_News_Block_Adminhtml_Category_Tree
{
    protected $_categoryIds = null;
    protected $_selectedNodes = null;

    /**
     * Constructor, specify template to use
     *
    ...
    {
        parent::__construct();
        $this->setTemplate('oggetto_news/news/edit/tab/category.phtml');
    }

    /**
     * Retrieve currently edited news
     *
    ...
    /**
     * Return array with IDs which the news is linked to
     *
     * @return array
     */
    public function getCategoryIds()
    {
        if (is_null($this->_categoryIds)) {
            $categories = $this->getNews()->getSelectedCategories();
            $ids = array();
    ...
    {
        $result = false;
        // Contains string with all news category IDs of children (not exactly direct) of the node
    ...
    /**
     * Returns array with nodes those are selected (contain current news)
     *
     * @return array
     */
     ...
     *
     * @param mixed $rootId Root news category Id for context
     * @return array
     */
    public function getSelectedCategoryPathIds($rootId = false)
    {
        ...
        $collection = Mage::getResourceModel('oggetto_news/category_collection');
        if ($rootId) {
    ...
    public function buildNodeName($node)
    {
        ...
        $result .= '<a target="_blank" href="' . $this->getUrl('adminhtml/news_category/index', array('id' => $node->getId(), 'clear' => 1)) . '"><em>' . $this->__(' - Edit') . '</em></a>';
        return $result;
    }
}
